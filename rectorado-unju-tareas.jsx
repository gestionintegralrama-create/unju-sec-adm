...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setDb(res ? JSON.parse(res.value) : { areas: SEED_AREAS, users: SEED_USERS, tasks: SEED_TASKS });
      } catch {
        if (!cancelled) setDb({ areas: SEED_AREAS, users: SEED_USERS, tasks: SEED_TASKS });
      } finally {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        await window.storage.set(STORAGE_KEY, JSON.stringify(db), true);
        setSaveState("saved");
      } catch {
        setSaveState("error");
      }
    ...
    <div className="app-root">
      ...
      {!db ? (
        ...
        <>
          ...
          <p className="storage-note">Los datos se guardan online y son compartidos por todos los perfiles de esta demo.</p>
        </>
      )}
    </div>
  );
