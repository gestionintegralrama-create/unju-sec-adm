...
function DireccionView({ db, actions }) {
  ...
  const perAreaStatusData = db.areas.map((area) => {
    const areaTasks = withStatus.filter((t) => t.areaId === area.id);
    return {
      area,
      data: Object.entries(STATUS).map(([k, v]) => ({
        key: k,
        status: v.label,
        count: areaTasks.filter((t) => t.status === k).length,
        color: v.color,
      })),
    };
  });
  ...
  const avanceData = db.areas.map((area) => {
    const areaTasks = withStatus.filter((t) => t.areaId === area.id);
    const avance = areaTasks.length
  ...
  const drillTasks = useMemo(() => {
    ...
    if (drill.type === "area-status") return withStatus.filter((t) => t.areaId === drill.value.areaId && t.status === drill.value.status);
    return [];
  }, [drill, withStatus]);
    ...
    <div className="view">
      ...
      {tab === "panorama" && !drill && (
        <>
          ...
          <div className="panel">
            <h3>Tareas por estado, separadas por área</h3>
            <p className="muted small">Cada área tiene su propio gráfico. Tocá una barra para ver el detalle.</p>
            <div className="area-charts-grid">
              {perAreaStatusData.map(({ area, data }) => (
                <div className="area-chart-card" key={area.id}>
                  <h4>{area.name}</h4>
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={data} margin={{ left: -10, right: 10, top: 5, bottom: 0 }}>
                      ...
                      <XAxis dataKey="status" stroke={PALETTE.inkSoft} fontSize={10.5}
                        interval={0} angle={-20} textAnchor="end" height={50} />
                      <YAxis allowDecimals={false} stroke={PALETTE.inkSoft} fontSize={11} width={28} />
                      <Tooltip />
                      <Bar dataKey="count" radius={[3, 3, 0, 0]} cursor="pointer"
                        onClick={(d) => setDrill({ type: "area-status", value: { areaId: area.id, status: d.key }, label: `${area.name} — ${d.status}` })}>
          ...
          <div className="two-col-layout">
            ...
            <div className="panel">
              ...
              <p className="muted small">Porcentaje de avance promedio de las tareas de cada área.</p>
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={avanceData} margin={{ left: -10, right: 20, top: 10, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={PALETTE.line} vertical={false} />
                  <XAxis dataKey="area" stroke={PALETTE.inkSoft} fontSize={11}
                    interval={0} angle={-20} textAnchor="end" height={55} />
                  <YAxis domain={[0, 100]} stroke={PALETTE.inkSoft} fontSize={12} width={32} unit="%" />
                  <Tooltip formatter={(v) => [`${v}%`, "Avance"]} labelFormatter={(_, p) => p?.[0]?.payload?.fullArea || ""} />
                  <Line type="monotone" dataKey="avance" stroke={PALETTE.secondary} strokeWidth={2.5}
                    dot={{ r: 5, fill: PALETTE.secondary, cursor: "pointer" }}
                    activeDot={{
                      r: 7, cursor: "pointer",
                      onClick: (_, d) => setDrill({ type: "area", value: d.payload.areaId, label: d.payload.fullArea }),
                    }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
...
.area-charts-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 14px; margin-top: 12px; }
.area-chart-card { background: var(--surface-soft); border: 1px solid var(--line); border-radius: 6px; padding: 10px 12px 4px; }
.area-chart-card h4 { margin: 0 0 4px; font-size: 12.5px; font-weight: 700; color: var(--ink); }
