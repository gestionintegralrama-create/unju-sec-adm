...
function DireccionView({ db, actions }) {
  ...
  const statusData = Object.entries(STATUS).map(([k, v]) => ({
    key: k, status: v.label, count: withStatus.filter((t) => t.status === k).length, color: v.color,
  }));

  const areaTotalsData = db.areas.map((area) => ({
    areaId: area.id, area: area.name, total: withStatus.filter((t) => t.areaId === area.id).length,
  }));

  const areaDelaysData = db.areas.map((area) => ({
    areaId: area.id, area: area.name, demoradas: withStatus.filter((t) => t.areaId === area.id && t.status === "demorada").length,
  }));

  const areaProgressData = db.areas.map((area) => {
    const areaTasks = withStatus.filter((t) => t.areaId === area.id);
    const avg = areaTasks.length ? Math.round(areaTasks.reduce((sum, t) => sum + t.progress, 0) / areaTasks.length) : 0;
    return { areaId: area.id, area: area.name, avance: avg };
  });
    ...
    <div className="view">
      ...
      {tab === "panorama" && !drill && (
        <>
          ...
          <div className="two-col-layout">
            <div className="panel">
              <h3>Tareas por estado (unificado)</h3>
              <p className="muted small">Todas las áreas juntas. Tocá una barra para ver el detalle.</p>
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={statusData} layout="vertical" margin={{ left: 10 }}>
                  ...
                  <YAxis type="category" dataKey="status" stroke={PALETTE.inkSoft} fontSize={11.5} width={130} />
                  <Tooltip />
                  <Bar dataKey="count" radius={[0, 3, 3, 0]} cursor="pointer"
                    onClick={(d) => setDrill({ type: "status", value: d.payload.key, label: d.payload.status })}>
            ...
            <div className="panel">
              ...
              <p className="muted small">Todas las áreas juntas. Tocá una porción para ver el detalle.</p>
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
          ...
          <h3 className="section-title">Desglose por área</h3>
          <div className="three-col-layout">
            <div className="panel">
              <h3>Tareas totales por área</h3>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={areaTotalsData} layout="vertical" margin={{ left: 10 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={PALETTE.line} horizontal={false} />
                  <XAxis type="number" allowDecimals={false} stroke={PALETTE.inkSoft} fontSize={11.5} />
                  <YAxis type="category" dataKey="area" stroke={PALETTE.inkSoft} fontSize={11} width={140} />
                  <Tooltip />
                  <Bar dataKey="total" fill={PALETTE.secondary} radius={[0, 3, 3, 0]} cursor="pointer"
                    onClick={(d) => setDrill({ type: "area", value: d.payload.areaId, label: d.payload.area })} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div className="panel">
              <h3>Tareas demoradas por área</h3>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={areaDelaysData} layout="vertical" margin={{ left: 10 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={PALETTE.line} horizontal={false} />
                  <XAxis type="number" allowDecimals={false} stroke={PALETTE.inkSoft} fontSize={11.5} />
                  <YAxis type="category" dataKey="area" stroke={PALETTE.inkSoft} fontSize={11} width={140} />
                  <Tooltip />
                  <Bar dataKey="demoradas" fill={PALETTE.alert} radius={[0, 3, 3, 0]} cursor="pointer"
                    onClick={(d) => setDrill({ type: "area", value: d.payload.areaId, label: `${d.payload.area} — demoradas` })} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div className="panel">
              <h3>Avance promedio por área</h3>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={areaProgressData} layout="vertical" margin={{ left: 10 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={PALETTE.line} horizontal={false} />
                  <XAxis type="number" domain={[0, 100]} unit="%" stroke={PALETTE.inkSoft} fontSize={11.5} />
                  <YAxis type="category" dataKey="area" stroke={PALETTE.inkSoft} fontSize={11} width={140} />
                  <Tooltip formatter={(v) => `${v}%`} />
                  <Bar dataKey="avance" fill={PALETTE.positive} radius={[0, 3, 3, 0]} cursor="pointer"
                    onClick={(d) => setDrill({ type: "area", value: d.payload.areaId, label: d.payload.area })} />
                </BarChart>
              </ResponsiveContainer>
            </div>
...
.three-col-layout { display: grid; grid-template-columns: repeat(3, 1fr); gap: 18px; margin-bottom: 18px; }
@media (max-width: 980px) { .three-col-layout { grid-template-columns: 1fr 1fr; } }
@media (max-width: 640px) { .three-col-layout { grid-template-columns: 1fr; } }
.section-title { font-size: 15px; margin: 22px 0 12px; color: var(--ink-soft); text-transform: uppercase; letter-spacing: 0.04em; font-family: 'Inter', sans-serif; font-weight: 700; }
